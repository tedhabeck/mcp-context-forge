...
class TestTokenCatalogService:
    ...
    async def test_list_user_and_team_tokens_basic(self, token_service, mock_db, mock_api_token):
        ...
        # Mock: no team memberships and one personal token
        mock_db.execute.return_value.all.return_value = []  # No team memberships
    ...
    @pytest.mark.asyncio
    async def test_list_user_and_team_tokens_with_team_admin(self, token_service, mock_db):
        """Test list_user_and_team_tokens includes team tokens for team admins."""
        # Create mock tokens
        ...
        # Mock team membership (user is admin of team-123)
        team_membership = MagicMock()
        team_membership.team_id = "team-123"
        mock_db.execute.return_value.all.return_value = [team_membership]

        # Mock returning both tokens
    ...
    async def test_list_user_and_team_tokens_with_pagination(self, token_service, mock_db):
        ...
        # Should execute with pagination params
        mock_db.execute.assert_called()

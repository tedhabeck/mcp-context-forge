...
import { closeModal } from "../../mcpgateway/admin_ui/modals.js";
...
beforeAll(() => {
  win = window;
  doc = document;
  realInitToolSelect = initToolSelect;
  // Ensure window.Admin exists for updateToolMapping/updateResourceMapping/updatePromptMapping
  window.Admin = window.Admin || {};
});

afterAll(() => {
  // jsdom environment is managed by vitest
});
...
beforeEach(() => {
  ...
  // Clear URL search params (including any team_id from previous tests)
...
describe("getEditSelections", () => {
  test("returns an empty Set for an unknown container ID", () => {
    const sel = getEditSelections("brand-new-container");
  ...
  test("returns the same Set on repeated calls for the same container", () => {
    const s1 = getEditSelections("my-container");
    s1.add("id-x");
    const s2 = getEditSelections("my-container");
    expect(s2.has("id-x")).toBe(true);
  });

  test("different containers hold independent Sets", () => {
    getEditSelections("container-a").add("shared-id");
    expect(getEditSelections("container-b").has("shared-id")).toBe(false);
  });
});
...
describe("resetEditSelections", () => {
  test("clears edit-server-tools, edit-server-resources, and edit-server-prompts", () => {
    getEditSelections("edit-server-tools").add("t1");
    getEditSelections("edit-server-resources").add("r1");
    getEditSelections("edit-server-prompts").add("p1");

    resetEditSelections();

    expect(getEditSelections("edit-server-tools").size).toBe(0);
    expect(getEditSelections("edit-server-resources").size).toBe(0);
    expect(getEditSelections("edit-server-prompts").size).toBe(0);
  });

  test("does NOT clear add-server container keys", () => {
    getEditSelections("associatedTools").add("t1");
    getEditSelections("associatedResources").add("r1");
    getEditSelections("associatedPrompts").add("p1");

    resetEditSelections();

    expect(getEditSelections("associatedTools").has("t1")).toBe(true);
    expect(getEditSelections("associatedResources").has("r1")).toBe(true);
    expect(getEditSelections("associatedPrompts").has("p1")).toBe(true);
  });
});
...
describe("ensureAddStoreListeners", () => {
  ...
  test("checking a tool checkbox adds its value to the Map", () => {
    ...
    expect(getEditSelections("associatedTools").has("tool-1")).toBe(true);
  });

  test("unchecking a tool checkbox removes its value from the Map", () => {
    setupContainers();
    ensureAddStoreListeners();
    getEditSelections("associatedTools").add("tool-1");
    ...
    expect(getEditSelections("associatedTools").has("tool-1")).toBe(false);
  });
  ...
  test("tracks resources and prompts checkboxes independently", () => {
    ...
    expect(getEditSelections("associatedResources").has("res-1")).toBe(true);
    expect(getEditSelections("associatedPrompts").has("prompt-1")).toBe(true);
    // Cross-container isolation
    expect(getEditSelections("associatedTools").has("res-1")).toBe(false);
  });
  ...
  test("form reset clears all add-server selections from the Map", () => {
    ...
    expect(getEditSelections("associatedTools").size).toBe(0);
    expect(getEditSelections("associatedResources").size).toBe(0);
    expect(getEditSelections("associatedPrompts").size).toBe(0);
  });
  ...
  test("form reset does not clear edit-server selections", () => {
    ...
    expect(getEditSelections("edit-server-tools").has("t1")).toBe(true);
  });
});
...
describe("serverSideToolSearch", () => {
  ...
  test("persists selection to store before empty-string search reloads container", async () => {
    ...
    // The empty-string path flushes DOM selections into the store before reloading
    expect(getEditSelections("associatedTools").has("t1")).toBe(true);
    expect(getEditSelections("associatedTools").has("t2")).toBe(false);

    // The container is reloaded with the server response HTML
    const container = doc.getElementById("associatedTools");
    expect(container.querySelector('input[value="t1"]')).not.toBeNull();
    expect(container.querySelector('input[value="t2"]')).not.toBeNull();
  });
  ...
  test("restores selections after keyword search results are rendered", async () => {
    ...
    await serverSideToolSearch("tool");
  ...
  test("accumulates selections across two successive searches", async () => {
    ...
    await serverSideToolSearch("one");
...
describe("serverSideResourceSearch", () => {
  ...
  test("flushes checked resource checkboxes into the Map before search", async () => {
    ...
    await serverSideResourceSearch("");
  ...
  test("restores selections after keyword search results are rendered", async () => {
    ...
    await serverSideResourceSearch("res");
...
describe("handleServerFormSubmit", () => {
  ...
  test("includes all tool IDs from the Map — including those from previous pages", async () => {
    ...
    getEditSelections("associatedTools").add("t1");
    getEditSelections("associatedTools").add("t2");
    ...
    await handleServerFormSubmit(fakeSubmitEvent(form));
...
describe("ensureEditStoreListeners", () => {
  ...
  test("checking an edit-server-tools checkbox adds its value to the Map", () => {
    ...
    expect(getEditSelections("edit-server-tools").has("tool-edit-1")).toBe(
      true
    );
  });
  ...
  test("unchecking an edit-server-tools checkbox removes its value from the Map", () => {
    setupEditContainers();
    ensureEditStoreListeners();
    getEditSelections("edit-server-tools").add("tool-edit-1");
  ...
  test("tracks edit-server resources and prompts independently", () => {
    ...
    const promptCb = addCheckbox(doc.getElementById("edit-server-prompts"), {
      name: "associatedPrompts",
      value: "prompt-e1",
    });
    promptCb.checked = true;
    ...
    expect(getEditSelections("edit-server-resources").has("res-e1")).toBe(true);
    expect(getEditSelections("edit-server-prompts").has("prompt-e1")).toBe(
      true
    );
    // Cross-container isolation
    expect(getEditSelections("edit-server-tools").has("res-e1")).toBe(false);
  });
...
describe("handleEditServerFormSubmit", () => {
  ...
  test("includes off-screen tool IDs from the edit store", async () => {
    ...
    getEditSelections("edit-server-tools").add("t1");
    getEditSelections("edit-server-tools").add("t2");
    ...
    await handleEditServerFormSubmit(fakeSubmitEvent(form));
...
describe("serverSideEditToolSearch", () => {
  ...
  test("flushes checked edit-mode checkboxes into the Map before search", async () => {
    ...
    await serverSideEditToolSearch("");
  ...
  test("restores selections from store after clear-search reload", async () => {
    const container = setupEditToolsContainer([{ value: "t1", checked: true }]);
  ...
  test("restores selections from store after keyword search", async () => {
    ...
    await serverSideEditToolSearch("tool");
...
describe("View Public checkbox — search function integration", () => {
  ...
  describe("serverSideToolSearch respects View Public state", () => {
    ...
    test("keyword search includes team_id when View Public is unchecked", async () => {
      ...
      await serverSideToolSearch("myTool");
  ...
  describe("serverSidePromptSearch respects View Public state", () => {
    ...
    test("keyword search includes team_id when View Public is unchecked", async () => {
      ...
      await serverSidePromptSearch("myPrompt");
  ...
  describe("serverSideResourceSearch respects View Public state", () => {
    ...
    test("keyword search includes team_id when View Public is unchecked", async () => {
      ...
      await serverSideResourceSearch("myRes");
  ...
  describe("serverSideEditToolSearch respects View Public state", () => {
    ...
    test("keyword search includes team_id when View Public is unchecked", async () => {
      ...
      await serverSideEditToolSearch("myTool");
  ...
  describe("serverSideEditPromptsSearch respects View Public state", () => {
    ...
    test("keyword search includes team_id when View Public is unchecked", async () => {
      ...
      await serverSideEditPromptsSearch("myPrompt");
  ...
  describe("serverSideEditResourcesSearch respects View Public state", () => {
    ...
    test("keyword search includes team_id when View Public is unchecked", async () => {
      ...
      await serverSideEditResourcesSearch("myRes");
...
describe("editServer visibility coercion when ALLOW_PUBLIC_VISIBILITY is false", () => {
  ...
  test("public server stays public when no teamId in URL (global scope)", async () => {
    ...
    await flagWin.Admin.editServer("srv-2");
  ...
  test("team visibility still selects team radio when flag is false", async () => {
    ...
    await flagWin.Admin.editServer("srv-3");
...
describe("editServer restores OAuth settings from camelCase API response (#3405)", () => {
  ...
  test("populates OAuth checkbox and config from camelCase response", async () => {
    ...
    const section = oauthDoc.getElementById("edit-server-oauth-config-section");
  ...
  test("snake_case properties are NOT read (regression guard)", async () => {
    ...
    mockFetch({
      ...
      oauth_enabled: true,
      oauth_config: {
        authorization_servers: ["https://should-not-appear.example.com"],
...
describe("closeModal — cleanup failure does not prevent modal hiding (#3259)", () => {
  test("modal hides even when resetEditSelections throws", () => {
    ...
    expect(() => closeModal("server-edit-modal")).not.toThrow();
  ...
  test("AppState.setModalInactive is called even when cleanup throws", () => {
    ...
    modal.id = "server-edit-modal";
    doc.body.appendChild(modal);
    AppState.setModalActive("server-edit-modal");
    ...
    closeModal("server-edit-modal");
    expect(AppState.isModalActive("server-edit-modal")).toBe(false);
...
describe("serverSideEditToolSearch — add-only flush (#3260)", () => {
  ...
  test("unchecked search-result checkboxes do NOT delete prior selections from store", async () => {
    ...
    getEditSelections("edit-server-tools").add("tool-previously-selected");
    ...
    await serverSideEditToolSearch("checked");
...
describe("serverSideEditPromptsSearch — add-only flush (#3260)", () => {
  ...
  test("unchecked search-result checkboxes do NOT delete prior prompt selections", async () => {
    ...
    getEditSelections("edit-server-prompts").add("prompt-prior");
    ...
    await serverSideEditPromptsSearch("checked");
...
describe("serverSideEditResourcesSearch — add-only flush (#3260)", () => {
  ...
  test("unchecked search-result checkboxes do NOT delete prior resource selections", async () => {
    ...
    getEditSelections("edit-server-resources").add("res-prior");
    ...
    win.fetch = vi.fn().mockResolvedValue(
      mockResponse({
        ...
        contentType: "application/json",
        body: {
          resources: [{ id: "res-checked", name: "Checked Resource" }],
        },
      })
    );

    await serverSideEditResourcesSearch("checked");
...
describe("initToolSelect Select All populates in-memory store (#3257)", () => {
  ...
  test("Select All populates getEditSelections store for the container", async () => {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if (getEditSelections("edit-server-tools").size >= 3) break;
    }

    const editSel = getEditSelections("edit-server-tools");
...
describe("ensureNoResultsElement", () => {
  ...
  test("creates element dynamically when missing from DOM", () => {
    ...
    expect(result.msg.className).toBe("text-gray-700 dark:text-gray-300 mt-2");

/**
 * Unit tests for admin.js HTML generation functions.
 *
...
describe("createSystemSummaryCard", () => {
    const f = () => win.createSystemSummaryCard;
...
describe("createKPISection", () => {
    ...
    test("handles non-numeric avgResponseTime", () => {
        const section = f()({ avgResponseTime: "not-a-number" });
        expect(section.textContent).toContain("N/A");
    });

    test("returns safe fallback on error", () => {
        const section = f()(null);
...
describe("createPerformanceCard", () => {
    const f = () => win.createPerformanceCard;
...
describe("createRecentActivitySection", () => {
    const f = () => win.createRecentActivitySection;

    test("displays activity items", () => {
        const section = f()([
            { action: "Created Tool", target: "get-weather", timestamp: "2025-01-01" },
            { action: "Deleted Resource", target: "old-config", timestamp: "2025-01-02" },
        ]);
        const text = section.textContent;
    ...
    test("applies escapeHtml to user data", () => {
        const section = f()([
            { action: "<script>alert('xss')</script>", target: "t", timestamp: "now" },
        ]);
